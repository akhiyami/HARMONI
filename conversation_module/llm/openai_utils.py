...
def generate_answer(question, history, conn=None, current_user=None):
    """
    ...
    completion = client.chat.completions.create(
        model="gpt-4o",
        messages=[reply_prompt, ltm, *stm, {"role": "user", "content": question}],
    )
...
def update_memory_llm(user_question, history, conn=None, current_user=None, database=None):
    ...
    # On envoie l’échange récent au LLM pour analyse
    ltm = {
        "primary_features": [],
        "features": []
    }

    # access the ltm through the database
    cursor = conn.cursor()
