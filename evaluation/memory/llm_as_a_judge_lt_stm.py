...
def process_speaker_interactions(
    ...
    for question, dia_id in tqdm(speaker_interactions, desc="Processing interactions", leave=False):
        ...
        new_memory_blob = update_memory_llm(question, conn=conn, current_user=speaker_id, stm=stm)
...
if __name__ == "__main__":
    ...
    for id in range(len(locomo_data)):
        ...
        scores_df_a.to_csv(f"results/locomo/ltm_stm/scores_{id}_a.csv", index=False)
        with open(f"results/locomo/ltm_stm/sessions_observations_{id}_a.json", "w") as f:
        ...
        scores_df_b.to_csv(f"results/locomo/ltm_stm/scores_{id}_b.csv", index=False)
        with open(f"results/locomo/ltm_stm/sessions_observations_{id}_b.json", "w") as f:
