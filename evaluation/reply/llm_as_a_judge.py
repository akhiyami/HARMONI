...
PROMPT_JUDGE_SYSTEM = {
    ...
    "content": (
        ...
        "Tu dois comparer deux réponses à la même question, et fournir pour chacune un score de 0 à 10 "
        "(0 = complètement inappropriée, 10 = excellente)"
        "Tu peux utiliser des exemples de bonnes réponses (chosen) et de mauvaises réponses (rejected) comme référence."
...
Meilleure réponse: Chosen answer
Raisonnement: La réponse choisie est plus complète, structurée et adaptée à l'utilisateur.

Maintenant, évalue ces deux réponses pour la question ci-dessous en donnant un score 0-10 et une justification pour chacune:
User profile: {user_profile}
Question: {question}
Réponse A (avec mémoire): {answer_memory}
Réponse B (sans mémoire): {answer_no_memory}
"""
...
def evaluate_with_llm(question, answer_memory, answer_no_memory, chosen, rejected, user_profile):
    """
...
def evaluate_example(example, conn):
    ...
    answer, _ = generate_answer(question, current_session, context, conn, current_user, visual_profile, reply_prompt=REPLY_PROMPT_EVAL, verbose=False)
    ...
    # LLM scoring
    llm_judge_output = evaluate_with_llm(question, answer, answer_no_memory, chosen, rejected, user_profile=user_profile)
    ...
    return {
        ...
        "llm_score_memory": llm_judge_output.score_a,
        "llm_score_no_memory": llm_judge_output.score_b,
